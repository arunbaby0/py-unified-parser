...
/* and deletions.                                      */
/* NOTE: if the key sent to hash(key,...) contains     */
/* more characters than there are parameter values in  */
/* the hash fcn (hpars->nparams), only that many are   */
/* used and a warning is sent to stderr.               */
...
/* code */
 ...
 if (!streq(key,"*"))
   {usable_table_length = table_length - 1;
    ...
    if (j >= hpars->nparams)
      {fprintf(stderr,"*ERR: hash fcn tried to use too many parameters.\n");
       fprintf(stderr,"  nparams=%d, but tried to use %d\n",hpars->nparams,j+1);
