...
#include "stdcenvf.h" /* std compile environment for functions */
  ...
  /*          15 iff alphabetic && not sorted && items > 200  */
  ...
  /* Revised 3/20/95 by WMF to merely check on order of large */
  /*  alphabetic files.                                       */
  /************************************************************/
  ...
  /* close file & adjust some variables  */
    ...
    if (!p1->hashed)
      {if ((streq(p1->order,"alphabetic"))||(streq(p1->order,"alpha")))
         {if (p1->ncodes <= 200) sort_pcodeset_a(p1);
          else
            {if (pcodeset_misorders(p1) > 0)
               {fprintf(stderr,
                  "%s: *ERR: pcodeset in file '%s' not sorted\n",proc,fname);
                fprintf(stderr,
                  "  although it is declared to be alphabetic.\n");
                *perr = 15;
                goto RETURN;
      }  }  }  }
