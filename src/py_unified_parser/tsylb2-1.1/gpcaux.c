...
if (db_level > 2) printf("%schecking for RULES keyword\n",pdb);
       if (strstr(upr_pline,"RULES") != NULL)
         {
if (db_level > 2) printf("%sRULES aux line being processed.\n",pdb);
          if (strstr(upr_pline,"STANDARD") != NULL)
            {
if (db_level > 2) printf("%sRULES STANDARD aux line being processed.\n",pdb);
...
/* NOTE: calloc inits to all zero bytes, so strlen(*)=0 */
 ...
 db_leave_msg(proc,3); /* debug only */
  ...
  /***************************************************************/
  /*  int pcodeset_misorders(p1) pcodeset *p1;                   */
  /*  Calculates the number of elements of pcodeset *p1 that are */
  /* not in alphabetic order.                                    */
  /***************************************************************/
  int pcodeset_misorders(pcodeset *p1)
  {int nbad,i;
